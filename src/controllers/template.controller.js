...
function TemplateController(TemplateRepository, APIError) {
    async function create(req, res, next) {
        try {
            const template_request = req.body;

            const new_template = await TemplateRepository.create(template_request);
    ...
    async function list(req, res, next) {
        try {
            const list_templates = await TemplateRepository.list(req.query);
    ...
    async function getTemplate(mail_options) {
        try {
            const template = await TemplateRepository.getByTemplateId(mail_options.template_id);
